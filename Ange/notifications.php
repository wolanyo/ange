...
<div id="zonetexte">
    <h2> <span style="font-size : 0.80em;">Liste des courriers ayant une
    priorit&eacute; : </span>
        <?php
    ...
    <p>
        <table cellpadding="0" cellspacing="0" border="0" id="tablo" >
            ...
            <?php
                ...
                if( $codeNotif == "r"){
                    $requete = $bd->prepare("SELECT * FROM courrier WHERE suite = 'Aucun' AND DATEDIFF( dateretourfixer, CURDATE() ) BETWEEN :p1 AND :p2 AND supprimer = 0 ORDER BY idcourrier DESC") ;
                ...
                while( $donnees = $requete->fetch() ){
                    ?>
                        <tr <?php if($rowsColor%2==0) echo 'class="lignepaire"'; else echo 'class="ligneimpaire"' ?> >
                            ...
                            <td><?php echo reverseDate($donnees['dateretourfixer'],'-','-'); ?></td>
                        </tr>
                    <?php
                    $rowsColor++ ;
                }
